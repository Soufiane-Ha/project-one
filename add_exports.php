...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    try {
        ...
        $Appurement = $_POST['appurement'];
        $paye = $_POST['paye'];

        $conn->beginTransaction();

        /* $stmt = $conn->prepare("INSERT INTO facture (client, sn_compa, category_compa, pay_comp) 
         VALUES (?, ?, ?, ?)");
        ...
        $stmt = $conn->prepare("INSERT INTO orders (id_product, n_facture, type, client, regime, type_input, bureadiwan, dec_reg_prec, num_declaration, s_n, designation, date_declaration, Appurement, paye) 
                                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->execute([$products_id, $n_facture, $type, $client, $regime, $type_input, $bureadiwan, $DecRegPrec, $num_declaration, $s_n, $designation, $date_declaration, $Appurement, $paye]);
...
<body>
    <main>
            ...
            <form action="" method="post">
                <div>
                    ...
                    <div id="extra-fields" style="display: none; margin-top: 10px;">
                        ...
                        <?php
                        $stmt_n_rep = $conn->prepare("SELECT MAX(n_rep) AS max_rep FROM orders WHERE type = 'sigad' AND type_input = 'export'");
                    ...
                    <label>Appurement </label>
                    <select name="appurement" id="" required>
                        <option value="">--select appurement--</option>
                        <option value="global">Global</option>
                        <option value="partiel">Partiel</option>
                    </select>

                    <label>Number Declaration | رقم التصريح</label>
                    ...
                    <label>Declaration | تصريح</label>
                    <input type="text" name="declaration" required>
        ...
        <ul class="nav-list">
                    ...
                    <li><a style="background-color:red;" href="add_exports.php">Expot</a></li>
